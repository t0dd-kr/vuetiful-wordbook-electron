  ...
  .then(() => {
    setTimeout(function () {
      ...
      .then(data => {
        ...
        if(words[idx + 1]) getTranslate(++idx, cb)
        else {
          cb(translation)
        }
      })
...
function crawler (w, cb) {
  idx = 0
  words = w
  translation = []
  // let binaryPath = app.getAppPath().replace('default_app','electron')
  // if (process.env.NODE_ENV !== 'development') {
  //   // global.__static = require('path').join(__dirname, '/static').replace(/\\/g, '\\\\')
  //   binaryPath = app.getAppPath().replace('default_app','electron')
  // }
  driver = new webdriver.Builder().usingServer('http://localhost:9515').withCapabilities(chromeCapabilities).forBrowser('chrome').build();
  getTranslate(idx, cb)
}
...
function close() {
  ...
  // }, 5000)
  spawn('taskkill',['/im','chromedriver.exe','/f'])
}
